	...
	protected function content() {
		...
		<div class="merlin__content--transition">
			...
			<?php if (1 < count($this->import_files)) : ?>
				...
				<p id="import-notice" style="display:none; font-weight: bold;"></p>
				...
				<div class="merlin__select-control-wrapper"> 
					<select class="merlin__select-control js-merlin-demo-import-select">
						<?php foreach ($this->import_files as $index => $import_file) : ?>
							<option 
								...
								data-preview-image="<?php echo esc_url($import_file['import_preview_image_url']); ?>" 
								data-import-notice="<?php echo esc_attr($import_file['import_notice']); ?>">
					...
					<script>
						document.addEventListener('DOMContentLoaded', function() {
							...
							var importNotice = document.getElementById('import-notice');

							function updateDemoDetails() {
								var selectedOption = selectElement.options[selectElement.selectedIndex];
								var previewImageUrl = selectedOption.getAttribute('data-preview-image');
								var noticeText = selectedOption.getAttribute('data-import-notice');
								...
								if (noticeText) {
									importNotice.textContent = noticeText;
							...
							// Show details of the first option on load
							updateDemoDetails();

							// Update details on option change
							selectElement.addEventListener('change', updateDemoDetails);
						});
					</script> 
		...
		<form action="" method="post" class="<?php echo esc_attr($multi_import); ?>">
			...
			<footer class="merlin__content__footer">
				...
				class="merlin__button merlin__button--skip merlin__button--closer merlin__button--proceed">
					<?php echo esc_html($skip); ?>
				</a>

				<a id="skip" href="<?php echo esc_url($this->step_next_link()); ?>" 
				class="merlin__button merlin__button--skip merlin__button--proceed">
					<?php echo esc_html($skip); ?>
				</a>
 ...
 * Import revolution slider.
 *
 * @param array $files Array of paths to the revolution slider zip files.
 */
public function import_revolution_sliders( $files ) {
    if ( ! class_exists( 'RevSlider', false ) ) {
        $this->logger->error( __( 'Revolution Slider class does not exist. Make sure the plugin is installed and activated.', 'merlin-wp' ) );
        return 'failed';
    }
    ...
    foreach ( $files as $file ) {
        if ( ! empty( $file['file_path'] ) && file_exists( $file['file_path'] ) ) {
            try {
                $this->logger->info( __( 'Importing slider: ', 'merlin-wp' ) . $file['name'] );
                $response = $importer->importSliderFromPost( true, true, $file['file_path'] );
                
                if ( $response && !is_wp_error($response) ) {
                    $this->logger->info( __( 'Successfully imported slider: ', 'merlin-wp' ) . $file['name'] );
                    $imported++;
                } else {
                    $this->logger->error( __( 'Failed to import slider: ', 'merlin-wp' ) . $file['name'] );
                    if ( is_wp_error($response) ) {
                ...
                $this->logger->error( __( 'Exception while importing slider: ', 'merlin-wp' ) . $file['name'] );
                $this->logger->error( $e->getMessage() );
                $failed++;
            }
        } else {
            $this->logger->error( __( 'Slider file not found: ', 'merlin-wp' ) . $file['name'] );
            $failed++;
        }
    }

    $this->logger->info( sprintf( __( 'Revolution Slider import finished. Imported: %d, Failed: %d', 'merlin-wp' ), $imported, $failed ) );
	...
	public function get_import_files_paths( $selected_import_index ) {
		...
		if ( ! empty( $selected_import_data['import_customizer_file_url'] ) ) {
		 ...
		 // Get revolution slider import files. If defined!
		 if ( ! empty( $selected_import_data['import_rev_slider_file_url'] ) ) {
			$slider_items = array();
		
			// Setup filename paths to save the Slider Revolution content.
			foreach ( $selected_import_data['import_rev_slider_file_url'] as $index => $slider_item ) {
				// Check if $slider_item is a string (direct URL) or an array
				if (is_string($slider_item)) {
					$file_url = $slider_item;
					$name = 'Slider ' . ($index + 1);
				} elseif (is_array($slider_item) && isset($slider_item['file_url'])) {
					$file_url = $slider_item['file_url'];
					$name = isset($slider_item['name']) ? $slider_item['name'] : 'Slider ' . ($index + 1);
				} else {
					error_log('Invalid slider item format at index ' . $index);
					continue;
				}
				...
				// Download the file, if it's missing.
				if ( empty( $file_path ) ) {
					error_log('Attempting to download slider from URL: ' . $file_url);
					$file_path = $downloader->download_file( $file_url, $slider_filename );
				}
		
				// Check for errors and log them
				if ( is_wp_error( $file_path ) ) {
					error_log('Error downloading slider: ' . $file_path->get_error_message());
					$file_path = '';
				} elseif ( empty( $file_path ) ) {
					error_log('Empty file path after attempted download for slider: ' . $name);
				}
				...
				if ( !empty( $file_path ) ) {
					...
					error_log('Successfully added slider: ' . $name . ' with file path: ' . $file_path);
				}
			}
		
			// Set the sliders import file.
			$import_files['sliders'] = $slider_items;
			error_log('Total sliders added: ' . count($slider_items));
		}elseif ( ! empty( $selected_import_data['local_import_rev_slider_file'] ) ) {
        $slider_items = array();

        // Setup filename paths to save the Slider Revolution content.
        foreach ( $selected_import_data['local_import_rev_slider_file'] as $slider_item ) {
            if ( file_exists( $slider_item['file_path'] ) ) {
